class PerlinNoise
  ...
  def int(x : Int, low : Int, high : Int) : Int
    ...
    ((noise(x).to_s.split('.').last.reverse[1..18].to_i % (high + 1 - low)) + low)
  end

  def int(x : Int, y : Int, low : Int, high : Int) : Int
    raise "low must be lower than high" if low >= high
    (noise(x, y).to_s.split('.').last.reverse[1..18].to_i % (high+1 - low)) + low
  end

  def int(x : Int, y : Int, z : Int, low : Int, high : Int) : Int
    raise "low must be lower than high" if low >= high
    (noise(x, y, z).to_s.split('.').last.reverse[1..18].to_i % (high+1 - low)) + low
  end
